...
namespace GithubActors_WPF.Actors
{
    ...
    public class GithubCoordinatorActor : ReceiveActor
    {
        ...
        private Messages.RepoKey _currentRepo;
        ...
        {
            Receive<GithubCommanderActor.CanAcceptJob>( job => SendAbleToAcceptJob( Sender,  job.Repo ) );
            ...
            Receive<BeginJob>( job =>
            {
                ...
                _githubWorker.Tell( new Messages.RetryableQuery( new GithubWorkerActor.QueryStarrers( job.Repo ), 4 ) );
            } );
        }
        ...
        private void Working()
        {
            ...
            Receive<User[]>( users =>
            {
                ...
                foreach ( var user in users )
                    _githubWorker.Tell( new Messages.RetryableQuery( new GithubWorkerActor.QueryStarrer( user.Login ), 3 ) );
            } );

            Receive<GithubCommanderActor.CanAcceptJob>( job => SendUnableToAcceptJob(Sender, job.Repo ));
            ...
            Receive<Messages.RetryableQuery>( query => query.CanRetry, query => _githubWorker.Tell( query ) );

            //query failed, can't be retried, and it's a QueryStarrers operation - means the entire job failed
            Receive<Messages.RetryableQuery>( query => !query.CanRetry && query.Query is GithubWorkerActor.QueryStarrers, query =>
            {
                _receivedInitialUsers = true;
                foreach ( var subscriber in _subscribers )
                {
            ...
            Receive<Messages.RetryableQuery>( query => !query.CanRetry && query.Query is GithubWorkerActor.QueryStarrer,
